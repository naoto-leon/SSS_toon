...
$(function () {
     ...
     * Slideshow
     */
    $('.slideshow_2').each(function () {

    // 変数の準備
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        var $container = $(this),                                 // a
            $slideGroup = $container.find('.slideshow-slides_2'),   // b
            $slides = $slideGroup.find('.slide_2'),                 // c
            $nav = $container.find('.slideshow-nav_2'),             // d
            $indicator = $container.find('.slideshow-indicator_2'), // e
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        function updateNav () {
            var $navPrev = $nav.find('.prev_2'), // Prev (戻る) リンク
                $navNext = $nav.find('.next_2'); // Next (進む) リンク
            // もし最初のスライドなら Prev ナビゲーションを無効に
            if (currentIndex === 0) {
                $navPrev.addClass('disabled_2');
            } else {
                $navPrev.removeClass('disabled_2');
            }
            // もし最後のスライドなら Next ナビゲーションを無効に
            if (currentIndex === slideCount - 1) {
                $navNext.addClass('disabled_2');
            } else {
                $navNext.removeClass('disabled_2');
            }
            // 現在のスライドのインジケーターを無効に
            $indicator.find('a').removeClass('active_2')
                .eq(currentIndex).addClass('active_2');
        }
